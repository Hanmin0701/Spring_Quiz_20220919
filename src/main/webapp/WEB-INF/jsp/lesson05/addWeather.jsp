<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>

  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">

  <script src="https://code.jquery.com/jquery-3.6.0.js" integrity="sha256-H+K7U5CnXl1h5ywQfKtSj8PCmoN9aaq30gDh27Xc0jk=" crossorigin="anonymous"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>

  <link rel="stylesheet" type="text/css" hrf="/css/lesson05/style.css">
</head>
<body>
	<img alt="img" src="/img/lesson05/weather">
	
	<div>
		<header class="bg-info">
			<logo class="bg-info"/>
		</header>
		
		<section class="bg-info">
			<!-- 왼쪽 section -->
			<div>
				 <aside>
                    <ul class="nav flex-column">
                        <li class="nav-item"><a href="data:image/png;base64," class="nav-link"/></li>
                        <li class="nav-item font-weight-bold"><a href="#" class="nav-link">날씨</a></li>
                        <li class="nav-item font-weight-bold"><a href="#" class="nav-link">날씨입력</a></li>
                        <li class="nav-item font-weight-bold"><a href="#" class="nav-link">테마날씨</a></li>
                        <li class="nav-item font-weight-bold"><a href="#" class="nav-link">관측기후</a></li>
                    </ul>
                </aside>
			</div>
		
			<!-- 오른쪽 section -->
			<div class="container">
				<h5>과거 날씨</h5>
				<hr>
				<table class="table table-striped">
					<thead>
						<tr>
							<td>날짜</td>
							<td>날씨</td>
							<td>기온</td>
							<td>강수량</td>
							<td>미세먼지</td>
							<td>풍속</td>
						</tr>
					</thead>
					<hr>
					<tbody>
					<c:forEach var="weatherInfo" items="weather" varStatus="status">
						<tr>
							<td><fmt:parseDate var="Date" value="${weather.date}" pattern="yyyy년 MM월 dd일" /></td>
							<td>${weather.weather}</td>
							<td>${weather.temperatures}</td>
							<td>${weather.precipitation}</td>
							<td>${weather.microDust}</td>
							<td>${weather.windSpeed}</td>
						</tr>
					</c:forEach>
					</tbody>
				</table>
			</div>
		</section>
		
		<footer>
			<div class="d-flex">
				<logo class="bg-info">
					<a herf="https://www.weather.go.kr/plus/resources/image/foot_logo.png"/>				
				</logo>
				<address class="text-secondary" class="bg-warning">
					(07062) 서울시 동작구 여의대방로 16길 61 <br>
					CopyRight@2020 KMA. All Rights RESERVED.				
				</address>
			</div>
		</footer>
	</div>
</body>
</html>